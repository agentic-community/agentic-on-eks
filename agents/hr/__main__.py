...
# def load_oauth_env():
#     """Load OAuth environment variables with proper defaults for HR agent."""
#     env_vars = {}
...
from hr_agent_taskmanager import HRAgentTaskManager
...
logger = logging.getLogger("hr_agent_api")
...
@click.option('--agentservice', 'agentservice', default='hr-agent-service.default.svc.cluster.local')
...
def main(host: str, port: int, agentservice: str, agentport: int, enable_oauth: bool):
    ...
    skill = AgentSkill(
        id='hr_agent',
        name='HR Agent',
        description='i can answer questions vacationdays employee name and details',
        tags=['HR Agent'],
        examples=['What is the name of employee EMP0002?', 'How many vacation days does employee EMP0002 have left?'],
    )
    ...
    if enable_oauth:
        logger.info(f"Configuring HR agent with OAuth authentication (enable_oauth={enable_oauth})")
        ...
        logger.info("OAuth security schemes configured for HR agent")
    else:
    ...
    agent_card = AgentCard(
        name='HR Agent',
        description='I am an HR agent , i can answer questions vacationdays employee name and details',
    ...
    if enable_oauth:
        logger.info("Configuring OAuth middleware for HR agent")
        try:
